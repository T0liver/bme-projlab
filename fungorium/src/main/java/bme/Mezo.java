...
public class Mezo implements Serializable {
  ...
  int milyenSzomszed(Mezo m) {
    ...
    if (Math.abs(mpos.get(0) - x) > 1
        || Math.abs(mpos.get(1) - y) > 1
        || (mpos.get(1) == y && mpos.get(0) == x)) return 0;
    if (tartozik != m.getTekton()) {
      for (int i = 0; i < fonalak.size(); ++i) if (fonalak.get(i).getVezet(this, m)) return 2;
      return 1;
    }
    for (int i = 0; i < fonalak.size(); ++i) if (fonalak.get(i).getVezet(this, m)) return 4;
    return 3;
  }
   ...
   */
  public void addFonal(
      GombaFonal gf) { // legyen inkább csak setter? ez nincs az uml diagramon - jó ez [Vid]
  ...
  public void fonalNo(GombaFonal melyik, Mezo honnan) {
    if (fonalak.contains(melyik)) return;
  ...
  public void fonalNovekszik(GombaFonal melyik, Mezo honnan) {
    ...
    if (milyenSzomszed(honnan) > 2) {
      if (melyik.getVezet(honnan, honnan)) { // Ellenőrzés, hogy szomszédról hidal-e át
        if (!fonalak.contains(melyik)) fonalak.add(melyik);
  ...
  List<Mezo> getSzomszedok() {
    ...
    for (int i = 0; i < terkep.getMezok().size(); ++i) {
      if (Math.abs(terkep.getMezok().get(i).getPos().get(0) - x) < 2
          && Math.abs(terkep.getMezok().get(i).getPos().get(1) - y) < 2) {
