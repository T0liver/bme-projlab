...
public class JatekosMenuView {
  ...
  /** A megjelenítendő játékos menü */
  private JatekosMenu jatekosMenu;

  /** A panel, amin megjelenik. */
  private JPanel panel;

  /** A játékvezérlő view, ahonnan az adatokat kapja. */
  ...
  /** Elérhető akciók panelje. */
  JPanel akcioPanel = new JPanel();

  /** Segítő szöveg megelenítésére. */
  JTextArea help;

  /** Kiválasztott akció */
  Akcio selectedAkcio;

  /** Kilépés/mentés panel */
   ...
   *
   * @param jatekosMenu a megjelenítendő játékos menü
   */
  ...
  /**
   * Publikus setter a játékvezérlőre.
   *
   * @param jvV a beálíltandó játékvezérlő
   */
  ...
  /** Körszámláló fissítése */
  public void frissitKorSzamlalo() {
    roundLabel.setText(
        "Kör: " + (jatekvezerloview.getJatekvezerlo().getJelenlegiKor() + 1) + "/50");
  }

  /**
   * Publikus setter a játékos menüre.
   *
   * @param jvV a beálíltandó játékos menü
   */
  ...
  /**
   * Játékos cseréje, amikor a következő játékos lép
   *
   * @param ujJatekos az új játékos.
   */
   ...
   * Kirajzolás
   *
   * @param w a JPanel, ahobá rajzoljon
   */
  ...
  /**
   * Középrehozott felirat elkészítése
   *
  ...
  /**
   * Színkontraszt különbség számolása
   *
   * @param c1 a felirat színe
   * @param c2 a háttér színe
   * @return true, ha eléggé elütnek a színek, hogy olvasható legyen a felirat a hátteren; false, ha
   *     nem.
   */
  private boolean colorContrast(Color c1, Color c2) {
    double r1c =
        (c1.getRed() / 255.0 <= 0.03928)
            ? c1.getRed() / 12.92
            : Math.pow((c1.getRed() + 0.055) / 1.055, 2.4);
    double g1c =
        (c1.getGreen() / 255.0 <= 0.03928)
            ? c1.getGreen() / 12.92
            : Math.pow((c1.getGreen() + 0.055) / 1.055, 2.4);
    double b1c =
        (c1.getBlue() / 255.0 <= 0.03928)
            ? c1.getBlue() / 12.92
            : Math.pow((c1.getBlue() + 0.055) / 1.055, 2.4);
    double luminance1 = 0.2126 * r1c + 0.7152 * g1c + 0.0722 * b1c;

    double r2c =
        (c2.getRed() / 255.0 <= 0.03928)
            ? c2.getRed() / 12.92
            : Math.pow((c2.getRed() + 0.055) / 1.055, 2.4);
    double g2c =
        (c2.getGreen() / 255.0 <= 0.03928)
            ? c2.getGreen() / 12.92
            : Math.pow((c2.getGreen() + 0.055) / 1.055, 2.4);
    double b2c =
        (c2.getBlue() / 255.0 <= 0.03928)
            ? c2.getBlue() / 12.92
            : Math.pow((c2.getBlue() + 0.055) / 1.055, 2.4);
