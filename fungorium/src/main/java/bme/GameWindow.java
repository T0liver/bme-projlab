...
public class GameWindow extends JFrame {
  ...
  public void altMenu() {
    ...
    gombaszokMinus.addActionListener(
        e -> {
          if (gombaszok > 2) {
            gombaszok--;
            updateLabels();
    ...
    gombaszokPlus.addActionListener(
        e -> {
          if (gombaszok < 5) {
            gombaszok++;
            updateLabels();
    ...
    rovaraszokMinus.addActionListener(
        e -> {
          if (rovaraszok > 2) {
            rovaraszok--;
            updateLabels();
    ...
    rovaraszokPlus.addActionListener(
        e -> {
          if (rovaraszok < 5) {
            rovaraszok++;
            updateLabels();
  ...
  public void saveGame() {
    ...
    try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(mentesMap))) {
      ...
      Save s =
          new Save(jw.getJelenlegiKor(), jw.getJelenlegiJatekos(), jw.jatekosok, jw.getTerkep());
   ...
   *
   * @return true, ha sikerült betölteni a játékállást; false ha nem.
   */
  public boolean loadGame() {
    ...
    if (mentettFajlok == null || mentettFajlok.length == 0) {
      JOptionPane.showMessageDialog(
          GameWindow.this, "Nem található mentés.", "Hiba", JOptionPane.ERROR_MESSAGE);
      return false;
    }

    // Fájlnevek kilistázása
    String[] fajlNevek = Arrays.stream(mentettFajlok).map(File::getName).toArray(String[]::new);
