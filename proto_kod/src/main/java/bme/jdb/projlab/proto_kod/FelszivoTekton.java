...
public class FelszivoTekton
     ...
     * eltűnteti az összes rajta lévő gombafonalat és spórát
     */
    @Override
    public void tick() {
        hatralevoido--;
        if (hatralevoido == 0) {
            for (int i = 0; i < fonalak.size(); ++i) {
                fonalak.get(i).removeFromTekton(this); // valahogy ki kell szedni erről a tektonról, majd az általa
            ...
            for (int i = 0; i < sporak.size(); ++i) {
                sporak.get(i).csokken(sporak.get(i).getDarabszam()); // a spórákat is felszívja specifikáció szerint
            }
